@extends('layouts.app')
@section('title', 'Dyeing Status Master')
@section('content')
<style>
</style>
<div class="row">
    <div class="col-12">
        <div class="card">
            <div class="card-body">
                <div class="row">
                    <div class="col-lg-4">
                        <h4 class="card-title">Dyeing Status Master</h4>
                    </div>
                </div>
            </div>
      </div>
                ...
                <center>
                    <div class="col-lg-6">
                        ...
                        <input type="hidden" name="hidden_id" id="hidden_id" />
                           <input class="form-control me-auto" name="dyeing_status" id="dyeing_status" type="text" placeholder="Enter Dyeing Status"
                                aria-label="Enter Dyeing Status">
      ...
      <div class="card">
                                    ...
                                    <div class="card-body centered">
                                        <table id="datatable" class="table table-bordered table-responsive">
                                                ...
                                                <tbody class="dt-responsive">
                                                    @foreach($dyeingstatus as $row)
                                                        <tr>
                                                            <td>{{$row->dyeing_status}}</td>
        ...
        <script>
            ...
            $("#btn-submit").click(function(e) {
                e.preventDefault();
                var name = $("#dyeing_status").val();
                // alert(name);
                var hidden_id = $("#hidden_id").val();
                if (name == '') {
                    toastr.warning("Please Enter Dyeing Status name");
                } else {
                    $.ajax({
                        type: 'POST',
                        url: "{{ route('dyiengstatus.store') }}",
                        data: {
                            _token: "{{ csrf_token() }}",
                            dyeing_status: name,
                // alert(dyeing_status);
                ...
                $.ajax({
                    ...
                    success: function(data) {
                         console.log("mydata",data);
                          ...
                          $('#dyeing_status').val(data['dyeing_status']);
